...
import { HomeIcon, CalendarDaysIcon, TrophyIcon, AcademicCapIcon, Bars3Icon, XMarkIcon } from '@heroicons/react/24/outline';
...
export default function Sidebar() {
  ...
  const menuItems = [
    ...
    { name: 'Academy', icon: <AcademicCapIcon className="h-5 w-5" /> },
    { name: 'Challange', icon: <TrophyIcon  className="h-5 w-5" /> },
    { name: 'Event', icon: <CalendarDaysIcon className="h-5 w-5" /> },
  ];
        ...
        <nav className="flex-1 px-4 py-2 space-y-1">
          {menuItems.map(({ name, icon }) => (
            <button
              ...
              className={`flex items-center w-full px-4 py-2 text-sm font-medium rounded-lg transition-all duration-300 ${activeItem === name ? 'bg-blue-500 text-white shadow' : 'text-gray-700 hover:bg-blue-100'}`}
            >
              {icon}
