...
export default function Pelatihan() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-blue-50 to-cyan-50 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="bg-white rounded-2xl shadow-lg border border-blue-100 p-6 mb-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h1 className="text-3xl font-bold text-blue-800 mb-2">Kelola Pelatihan</h1>
              <p className="text-blue-600">Manajemen program pelatihan dan pengajar</p>
            </div>
            <button 
              onClick={() => handleOpen()} 
              className="flex items-center gap-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white px-6 py-3 rounded-xl shadow-lg transition-all duration-300 transform hover:scale-105"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="bg-white rounded-xl shadow-md border border-blue-100 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-blue-600">Total Pelatihan</p>
                <p className="text-2xl font-bold text-blue-800">{pelatihan.length}</p>
              </div>
            </div>
          </div>
            ...
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-blue-100">
                {pelatihan.map((item, index) => (
                  <tr key={item.id_pelatihan} className="hover:bg-blue-50 transition-colors duration-200">
                    <td className="px-6 py-4 text-sm text-blue-700 font-medium">
                      {(page - 1) * limit + index + 1}
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm font-semibold text-blue-800">{item.judul}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-blue-600 max-w-xs truncate" title={item.deskripsi}>
                        {item.deskripsi}
                      </div>
                    </td>
                    <td className="px-6 py-4">
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className="px-4 py-2 text-sm font-medium text-blue-600 bg-white border border-blue-200 rounded-lg hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
                >
                  Sebelumnya
                </button>
                <button 
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6">
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-blue-700 mb-2">Judul Pelatihan</label>
                  <input 
                    type="text" 
                    placeholder="Masukkan judul pelatihan" 
                    value={form.judul} 
                    onChange={(e) => setForm({ ...form, judul: e.target.value })} 
                    className="w-full p-3 border border-blue-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200" 
                    required 
                  />
                </div>
                
                <div>
                  <label className="block text-sm font-medium text-blue-700 mb-2">Deskripsi</label>
                  <textarea 
                    placeholder="Masukkan deskripsi pelatihan" 
                    value={form.deskripsi} 
                    onChange={(e) => setForm({ ...form, deskripsi: e.target.value })} 
                    className="w-full p-3 border border-blue-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors duration-200 resize-none" 
                    rows="3"
                    required 
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-blue-700 mb-2">Tanggal Pelatihan</label>
                  <input 
                    type="date" 
                    value={form.tanggal} 
                ...
                <div>
                  <label className="block text-sm font-medium text-blue-700 mb-2">Kuota Peserta</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="block text-sm font-medium text-blue-700 mb-2">Pilih Pengajar</label>
                  <select 
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-blue-100 mt-6">
                <button 
                  onClick={() => setOpenModal(false)} 
                  className="px-6 py-3 text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 rounded-xl font-medium transition-colors duration-200"
                >
                  Batal
                </button>
                <button 
                  onClick={handleSave} 
                  className="px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl font-medium shadow-lg transition-all duration-200 transform hover:scale-105"
                >
